...
export function spawnPrimaryShots(
  ...
  switch (primaryKey) {
    ...
    case "blazing_laser": {
      ...
      // Level 4+: Full power — wide beam spread like max-power Blazing Lazers, but still readable
      shots.push(
        createBullet(primaryKey, state, {
          x: state.x - 20,
          vx: -spread(level >= 6 ? 68 : 60, precisionRoute),
          radius: 3.4,
          length: level >= 6 ? 48 : 46,
          vy: -speed(level >= 6 ? 1160 : 1140, precisionRoute),
          damage: damage(level >= 6 ? 0.5 : 0.48, level, precisionRoute, aggressiveRoute),
          pierce: 2,
          maxLife: 0.74,
        }),
        createBullet(primaryKey, state, {
          x: state.x - 8,
          radius: level >= 5 ? 4.2 : 4,
          length: level >= 5 ? 61 : 58,
          vy: -speed(level >= 5 ? 1265 : 1240, precisionRoute),
          damage: damage(level >= 5 ? 0.7 : 0.66, level, precisionRoute, aggressiveRoute),
          pierce: level >= 5 ? 5 : 4,
          maxLife: 0.7,
        }),
        createBullet(primaryKey, state, {
          x: state.x,
          radius: level >= 6 ? 5.2 : level >= 5 ? 5.1 : 5,
          length: level >= 6 ? 74 : level >= 5 ? 71 : 68,
          vy: -speed(level >= 6 ? 1335 : level >= 5 ? 1315 : 1300, precisionRoute),
          damage: damage(level >= 6 ? 0.96 : level >= 5 ? 0.94 : 0.92, level, precisionRoute, aggressiveRoute),
          pierce: level >= 6 ? 6 : 5,
          maxLife: 0.68,
        }),
        createBullet(primaryKey, state, {
    ...
    default: {
      ...
      shots.push(
        createBullet(primaryKey, state, {
          x: state.x - 30,
          vx: -spread(level >= 6 ? 300 : 280, precisionRoute),
          vy: -speed(level >= 6 ? 555 : 540, precisionRoute),
          damage: damage(level >= 6 ? 0.8 : 0.74, level, precisionRoute, aggressiveRoute),
          radius: 3.2,
          length: 12,
        }),
        createBullet(primaryKey, state, {
          x: state.x - 12,
          vx: -spread(level >= 5 ? 132 : 120, precisionRoute),
          vy: -speed(level >= 5 ? 645 : 620, precisionRoute),
          damage: damage(level >= 5 ? 1.04 : 0.96, level, precisionRoute, aggressiveRoute),
          radius: 3.9,
          length: 15,
        }),
        createBullet(primaryKey, state, {
          x: state.x,
          vx: 0,
          vy: -speed(level >= 6 ? 820 : level >= 5 ? 790 : 760, precisionRoute),
          damage: damage(level >= 6 ? 1.34 : level >= 5 ? 1.28 : 1.22, level, precisionRoute, aggressiveRoute),
          radius: level >= 6 ? 4.7 : level >= 5 ? 4.5 : 4.4,
          length: level >= 6 ? 24 : level >= 5 ? 22 : 20,
          pierce: level >= 6 ? 2 : level >= 5 ? 1 : 0,
        }),
        createBullet(primaryKey, state, {
          x: state.x + 12,
        ...
        ...(level >= 5 ? [
          createBullet(primaryKey, state, {
            x: state.x,
            y: state.y - state.radius - 16,
            vx: -spread(level >= 6 ? 34 : 24, precisionRoute),
            vy: -speed(level >= 6 ? 690 : 655, precisionRoute),
            damage: damage(level >= 6 ? 0.78 : 0.62, level, precisionRoute, aggressiveRoute),
            radius: 3.7,
            length: 14,
          }),
          createBullet(primaryKey, state, {
