...
export default function ShopScreen() {
  ...
  const updatePityAfterPull = (pulled: GachaResult[]) => {
    ...
    for (const r of pulled) {
      ...
      if (r.outfit.rarity === "SR" || r.outfit.rarity === "SSR") newPity.pullsSinceSR = 0;
      if (r.outfit.rarity === "SSR") newPity.pullsSinceSSR = 0;
    }
  ...
  const doPull = async (count: 1 | 10) => {
    ...
    const pulled = count === 1 ? [pullOne(save.ownedOutfits)] : pullTen(save.ownedOutfits);
    applyGachaResults(pulled);
    updatePityAfterPull(pulled);
    setIsRevealing(true);
    setResults(pulled);
    setPreviewResult([...pulled].sort((a, b) => RARITY_RANK[b.outfit.rarity] - RARITY_RANK[a.outfit.rarity])[0] ?? null);

    // SSR: full outfit reveal cutscene. SR: character cutin.
    const topPremium = [...pulled].sort((a, b) => RARITY_RANK[b.outfit.rarity] - RARITY_RANK[a.outfit.rarity])[0];
    if (topPremium && (topPremium.outfit.rarity === "SSR" || topPremium.outfit.rarity === "SR")) {
      const url = topPremium.outfit.rarity === "SSR"
        ? (topPremium.outfit.cutsceneArtUrl ?? topPremium.outfit.cutinUrl)
        : topPremium.outfit.cutinUrl;
      if (url) setActiveCutin({ id: Date.now(), url });
    }
  };

  const closeResults = () => { setResults(null); setIsRevealing(false); setPreviewResult(null); };
      ...
      {featured && (
        <section className="shop-featured panel-surface">
          ...
          <div className="shop-featured-content">
            <CardArt title={featured.name} artUrl={featured.artUrl} artPlaceholder={featured.artPlaceholder} rarity={featured.rarity} className="shop-featured-art" motionMode="never" />
            <div className="shop-featured-info">
              <strong className="shop-featured-name">{featured.name}</strong>
              <span className="rarity-text" style={{ color: RARITY_COLORS[featured.rarity] }}>{featured.rarity}</span>
      ...
      <div className="shop-layout">
        <section className="shop-pulls panel-surface">
          ...
          <div className="shop-buttons">
            <button className="btn btn-gacha" onClick={() => doPull(1)} disabled={spending || isRevealing || (!canAffordWithCredits(1) && !canAffordWithShared(1))}>
              <div className="gacha-btn-title">1-Pull</div>
              <div className="gacha-btn-cost">{shouldUseSharedForPull(1) ? `${SHARED_COST_1} HavnAI` : `${PULL_COST_1} Credits`}</div>
            </button>
            <button className="btn btn-gacha btn-gacha-10" onClick={() => doPull(10)} disabled={spending || isRevealing || (!canAffordWithCredits(10) && !canAffordWithShared(10))}>
              <div className="gacha-btn-title">10-Pull</div>
              <div className="gacha-btn-cost">{shouldUseSharedForPull(10) ? `${SHARED_COST_10} HavnAI` : `${PULL_COST_10} Credits`}</div>
            </button>
          </div>
        </section>
        <section className="shop-rates panel-surface">
          ...
          <p className="shop-note">SR/SSR pulls play outfit reveal cutscenes on pull.</p>
        </section>
      </div>
      ...
      <section className="shop-pity panel-surface">
        ...
        <div className="shop-pity-bars">
          <div className="shop-pity-row">
            ...
            <div className="shop-pity-track"><div className="shop-pity-fill shop-pity-fill-sr" style={{ width: `${srPityPct}%` }} /></div>
          ...
          <div className="shop-pity-row">
            ...
            <div className="shop-pity-track"><div className="shop-pity-fill shop-pity-fill-ssr" style={{ width: `${ssrPityPct}%` }} /></div>
              ...
              {results.map((r, i) => (
                <div key={i} className={`gacha-result-card rarity-${r.outfit.rarity.toLowerCase()}`} onClick={() => setPreviewResult(r)}>
                  ...
                  <div className="gacha-result-info">
                    ...
                    {r.isNew ? <span className="new-badge">NEW!</span> : <span className="shard-badge">+{r.shardsGained} shards</span>}
                  </div>
                </div>
              ))}
            </div>
            <button className="btn btn-primary" onClick={closeResults}>OK</button>
          </div>
        </div>
      )}
