    ...
    astraReward(wallet.address, shmupResult.score, grade, durationS, mapId ?? "unknown", runToken, wallet.sign)
      .then((res) => {
        if (res.ok && res.reward && res.reward > 0) {
          ...
          // The flywheel: a rewarded run also earns personalized pilot art,
          // generated by the HavnAI network from the run's own pilot,
          // outfit, map, and grade. Fire-and-forget -- the image lands in
          // the Collection gallery when a node completes the job.
